...
- 部分Lua脚本不一样，薄荷使用了雾凇的一些脚本，但是也有自己的脚本；同时，雾凇的一些脚本，薄荷也没有；同时，一些Lua可能功能一样，但是是使用不同的代码实现：
  - 星期、日期、时间：两者都提供此类功能，但默认触发编码和实现可能不同。当前薄荷使用 `oxq`、`orq`、`osj`，详见[特殊功能键](funcKeys.html)；不要直接沿用旧版的 `week`、`date`、`time` 引导；
  - 雾凇的 Unicode 功能偏向“输入 Unicode 码位得到文字”；薄荷的 `unicode_translator.lua` 则是“输入字词编码查询 Unicode 表示”，使用 `Uc` 前缀配合当前方案编码，输出 `U+4E2D`、`\u4E2D`、`&#x4E2D;` 等格式；
  - 薄荷为了限制输入的检索内容，使用了`codeLengthLimit_processor.lua`限制最大检索长度，雾凇没有；
